...
fn runtime_dmabuf_loopback_candidate(test_name: &str) -> Option<RuntimeDmabufLoopbackCandidate> {
    ...
    for physical_device in devices {
        ...
        for format in candidates {
            ...
            assert_eq!(
                renderer.capabilities().import.modifiers,
                renderer.capabilities().import.dmabuf
            );
            assert!(
                renderer
                    .capabilities()
...
#[test]
#[ignore = "requires a working Vulkan loader, physical device and dmabuf-exportable loopback format"]
fn runtime_import_sampled_dmabuf_samples_released_loopback_dmabuf() {
    ...
    let sampled_import = unsafe {
        ...
        // GENERAL layout, and its acquire SyncPoint represents that completed release. Constructing
        // the wrapper is the public contract boundary; generic ImportDma remains fail-closed below.
        VulkanSampledDmabufImport::foreign_general(
            &candidate.dmabuf,
            VulkanSampledDmabufAcquire::with_sync(Some(evidence.acquire_sync())),
        )
    };
    let texture = candidate
        .renderer
        .import_sampled_dmabuf(sampled_import)
...
fn runtime_import_dma_wl_loopback_samples_and_releases_with_drm_syncobj() {
    ...
    let sample_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        .expect("normal WaylandSurfaceRenderElement construction should import sampled loopback dmabuf")
        .expect("normal WaylandSurfaceRenderElement construction should create one sampled element");
        assert!(
            buffer.release_point().is_none(),
            "successful WaylandSurfaceRenderElement construction must take Wayland release ownership"
        );
    ...
    if let Err(payload) = sample_result {
        if !release_satisfied && buffer.release_point().is_none() {
...
fn runtime_import_dma_wl_loopback_reacquires_same_dmabuf_after_cache_release() {
    ...
    unsafe {
        ...
        // the first Wayland acquire point. There is no intervening use before from_surface's internal
        // import_surface call.
    ...
    let reacquire_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> bool {
        ...
        .expect("normal WaylandSurfaceRenderElement construction should import first sampled loopback dmabuf")
        .expect("normal WaylandSurfaceRenderElement construction should create one first sampled element");
        let first_buffer_ref = first_buffer
            .as_ref()
            .expect("test should retain first buffer until after initial sampled draw");
        assert!(
            first_buffer_ref.release_point().is_none(),
            "first WaylandSurfaceRenderElement construction must take Wayland release ownership"
        );
        ...
        let second_render_element = crate::wayland::compositor::with_states(&surface, |states| {
            crate::backend::renderer::element::surface::WaylandSurfaceRenderElement::from_surface(
                &mut candidate.renderer,
                &surface,
                states,
        ...
        .expect("normal WaylandSurfaceRenderElement construction should reacquire same sampled dmabuf")
        .expect(
            "normal WaylandSurfaceRenderElement construction should create one reacquired sampled element",
        );
        assert!(
            second_buffer.release_point().is_none(),
            "same-dmabuf WaylandSurfaceRenderElement construction must take the second Wayland release ownership"
        );
        second_cache_needs_release_for_cleanup = true;
        assert_eq!(
            candidate
        ...
        let second_render_elements = [second_render_element];
        runtime_draw_wayland_surface_elements_to_offscreen_and_assert_non_black(
            &mut candidate.renderer,
            &second_render_elements,
            render_format,
        ...
        Err(payload) => {
            if second_cache_needs_release_for_cleanup {
                ...
                let first_release_ownership_taken = first_buffer
                    ...
                    .map(|buffer| buffer.release_point().is_none())
                    .unwrap_or(true);
                if !first_release_satisfied && first_release_ownership_taken {
                    let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                        retire_import_wl_surface_textures_and_wait_for_tests(
                            &mut candidate.renderer,
                            &surface,
...
fn runtime_import_dma_wl_loopback_replaces_cached_dmabuf_with_fresh_contents() {
    ...
    unsafe {
        ...
        // the first Wayland acquire point. There is no intervening use before the first from_surface's
        // internal import_surface call.
    ...
    let post_first_import_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| -> bool {
        ...
        let first_buffer_ref = first_buffer
            .as_ref()
            .expect("test should retain first buffer until after first sampled draw");
        assert!(
            first_buffer_ref.release_point().is_none(),
        ...
        let second_buffer = second_buffer_for_cleanup
            .as_ref()
            .expect("replacement test should retain second buffer through second sampled draw");
        unsafe {
            ...
            // first cached texture on the same WlSurface; from_surface's internal import_surface call
            // must release it before importing this second buffer.
            admit_loopback_current_surface_commit_for_tests(
                &candidate.renderer,
                &surface,
                &second_buffer,
        ...
        assert!(
            first_release_point_probe.wait(0).is_err(),
            "first Wayland release point should not be signaled before replacement import releases the retired cache"
        );

        let second_render_element = match crate::wayland::compositor::with_states(&surface, |states| {
            crate::backend::renderer::element::surface::WaylandSurfaceRenderElement::from_surface(
                &mut candidate.renderer,
                &surface,
                states,
            ...
            Ok(Some(render_element)) => render_element,
            Ok(None) => panic!(
                "normal WaylandSurfaceRenderElement construction should create one replacement sampled element"
            ),
            Err(err) => {
                let cleanup_succeeded = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    retire_import_wl_surface_textures_and_wait_for_tests(
                        &mut candidate.renderer,
                        &surface,
                        &first_release_point_probe,
                ...
                panic!(
                    "normal replacement WaylandSurfaceRenderElement construction should release first sampled dmabuf and import second: {err:?}"
                );
            }
        };
        first_release_point_probe
        ...
        assert!(
            second_buffer.release_point().is_none(),
            "replacement WaylandSurfaceRenderElement construction must take the second Wayland release ownership"
        );
        second_cache_needs_release_for_cleanup = true;
        assert_eq!(
            candidate
...
fn runtime_import_dma_wl_replacement_failure_releases_cached_dmabuf() {
...

    {
        ...
        let mut frame = candidate
            .renderer
            .render(&mut first_target, (4, 4).into(), Transform::Normal)
            .expect("render first loopback dmabuf target before failed replacement");
        frame
            .clear(Color32F::new(0.625, 0.25, 0.875, 1.0), &full_damage)
            .expect("clear first loopback dmabuf render target before failed replacement");
    }
    ...
    let (first_acquire_point, first_release_point) =
        DrmSyncPoint::timeline_pair_for_tests(&drm_device, 51, 52)
            .expect("create first DRM syncobj acquire/release timeline points");
    if !import_or_signal_wayland_acquire_point_for_tests(
        test_name,
        first_evidence.acquire_sync(),
    ...
    let replacement_failure_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        let second_image = candidate
            .allocator
            .create_buffer_with_usage(4, 4, candidate.format.code, &[candidate.format.modifier], usage)
            .expect("allocate replacement dmabuf image");
        let second_dmabuf = second_image.export().expect("export replacement dmabuf");
        assert_eq!(second_dmabuf.format(), candidate.format);

        let (second_acquire_point, second_release_point) =
            DrmSyncPoint::timeline_pair_for_tests(&drm_device, 53, 54)
                .expect("create replacement DRM syncobj acquire/release timeline points");
        second_acquire_point
            .signal()
        ...
        first_release_point_probe
            .wait(1_000_000_000)
            .expect("failed replacement import should still signal first Wayland release point");
        first_release_satisfied_for_cleanup = true;
        assert!(
            second_buffer.release_point().is_some(),
            "failed replacement import must not consume the replacement Wayland release ownership"
        );
        assert!(
            second_release_point_probe.wait(0).is_err(),
            "failed replacement import must not signal the replacement release point"
        );
        ...
        crate::wayland::compositor::with_states(&surface, |states| {
            let data = states
                .data_map
                .get::<crate::backend::renderer::utils::RendererSurfaceStateUserData>()
                .expect("failed replacement should preserve renderer surface state");
            let data = data.lock().unwrap();
            assert!(
                data.texture(candidate.renderer.context_id()).is_none(),
                "failed replacement should leave no active cached Vulkan texture"
            );
        });
        assert_eq!(
            candidate
...
fn public_dmabuf_import_gates_formats() {
    ...
    let sampled_import = unsafe {
        ...
        // contract; import_sampled_dmabuf returns before any Vulkan import.
...
#[cfg(all(feature = "wayland_frontend", feature = "backend_drm"))]
#[test]
fn import_surface_texture_cache_lifecycle_marker_is_commit_local() {
    let mut renderer = VulkanRenderer::new_scaffold_for_tests();
    renderer.capabilities.formats.modifier_records = vec![modifier_record_from_properties(
        Fourcc::Abgr8888,
        vk::DrmFormatModifierPropertiesEXT {
    ...
    let (first_acquire_point, first_release_point) =
        DrmSyncPoint::invalid_timeline_pair_for_tests(135, 136).unwrap();
    let Some((display, _client_side, surface, first_buffer)) =
        import_surface_dmabuf_wl_surface_with_sync_points_for_tests(
            dmabuf.clone(),
            first_acquire_point,
    ...
    unsafe {
        // SAFETY: This unit test validates wrapper-local lifecycle storage only. It does not import,
        // sample, or release a Vulkan image with the constructed release-lifecycle assumption.
        renderer
            .mark_wayland_surface_current_dmabuf_commit_texture_cache_release_lifecycle_for_sampled_import(
                &surface, &dmabuf,
            )
    ...
    assert!(
        renderer
            .sampled_dmabuf_wayland_buffer_texture_cache_release_lifecycle(&first_buffer, &dmabuf)
            .unwrap()
            .is_some(),
        "first current-surface lifecycle marker should be stored only on the first renderer-managed buffer"
    );

    let display_handle = display.handle();
    let (second_acquire_point, second_release_point) =
        DrmSyncPoint::invalid_timeline_pair_for_tests(137, 138).unwrap();
    let expected_second_release_point = second_release_point.clone();
    let second_buffer = update_import_wl_surface_dmabuf_buffer_with_sync_points_for_tests(
        &display_handle,
        &surface,
    ...
    assert!(
        renderer
            .sampled_dmabuf_wayland_buffer_texture_cache_release_lifecycle(&second_buffer, &dmabuf)
            .unwrap()
            .is_none(),
        "release-lifecycle marker from a previous commit must not carry over to the replacement buffer"
    );
    unsafe {
        // SAFETY: This fixture supplies only external-state evidence for the second current buffer. It
        // deliberately omits fresh lifecycle coverage to prove a stale marker from the previous commit
        // cannot satisfy release call-site policy.
        renderer
            .assume_wayland_surface_current_dmabuf_commit_foreign_general_for_sampled_import(
                &surface, &dmabuf,
            )
    ...
    assert_buffer_release_point_matches_for_tests(
        &second_buffer,
        &expected_second_release_point,
        "commit-local lifecycle fixture should keep replacement release point before failed import",
    );
    ...
    assert_buffer_release_point_matches_for_tests(
        &second_buffer,
        &expected_second_release_point,
        "stale lifecycle marker guard must not consume replacement release ownership",
    );
    assert!(renderer.dmabuf_formats().iter().next().is_none());
}
...
fn import_surface_protocol_policy_rejects_missing_producer_contracts() {
    ...
    let same_view_other_wrapper = dmabuf_with_planes_for_tests(
        (1, 1).into(),
        Fourcc::Abgr8888,
        Modifier::Linear,
        DmabufFlags::empty(),
    ...
    let same_view_other_wrapper_proof =
        VulkanWaylandDmabufSampledImportViewProof::new(&dmabuf, &same_view_other_wrapper).unwrap();
    let unrelated_view =
        VulkanWaylandDmabufSampledImportViewProof::new(&unrelated_dmabuf, &unrelated_dmabuf).unwrap();
    assert!(matches!(
        VulkanWaylandDmabufSampledImportViewProof::new(&dmabuf, &unrelated_dmabuf),
        Err(VulkanError::UnsupportedOperation(
    ...
    assert!(matches!(
        unsafe {
            ...
            missing_view.admit_current_surface_commit(&renderer, &surface, &dmabuf)
        },
        Err(VulkanError::MissingCapability(
            "sampled dmabuf producer imported view"
        ))
    ...
    assert!(matches!(
        unsafe {
            ...
            mismatched_view.admit_current_surface_commit(&renderer, &surface, &dmabuf)
        },
        Err(VulkanError::UnsupportedOperation(
    ...
    assert!(matches!(
        unsafe {
            ...
            // bound to the current committed dmabuf wrapper.
            same_view_wrong_wrapper.admit_current_surface_commit(&renderer, &surface, &dmabuf)
        },
        Err(VulkanError::UnsupportedOperation(
            "sampled dmabuf producer imported view identity"
        ))
...
fn runtime_renderer_builder_initializes_with_first_physical_device() {
    ...
    assert_eq!(
        caps.import.dmabuf,
        caps.formats.dmabuf_import.iter().next().is_some()
    );
    assert_eq!(caps.import.modifiers, caps.import.dmabuf);
    assert!(renderer.dmabuf_formats().iter().next().is_none());
        ...
        .find(|record| record.usages.sampled && record.plane_count > 0)
    {
        ...
        assert_eq!(
            caps.formats
                .dmabuf_import
                .iter()
                .any(|format| { format.code == record.format && format.modifier == record.modifier }),
            record.usages.sampled && record.plane_count == 1
        );
    }
    assert!(caps.rendering.offscreen);
    assert!(!caps.rendering.blit);
    ...
    assert_eq!(
        ...
        caps.formats
            .modifier_records
            .iter()
            .any(|record| record.usages.sampled && record.plane_count == 1)
    );
    assert!(renderer.dmabuf_formats().iter().next().is_none());
    assert!(caps.formats.dmabuf_export.iter().next().is_none());
    assert!(
...
fn runtime_format_discovery_finds_device_backed_formats_without_import_export() {
    ...
    assert_eq!(
        caps.dmabuf_import.iter().next().is_some(),
        caps.modifier_records
            .iter()
            .any(|record| record.usages.sampled && record.plane_count == 1)
    );
    assert!(caps.dmabuf_import.iter().all(|format| {
        caps.modifier_records.iter().any(|record| {
            record.format == format.code
                && record.modifier == format.modifier
                && record.usages.sampled
                && record.plane_count == 1
        })
    }));
